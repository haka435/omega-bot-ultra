...
if df is not None and not df.empty and "Close" in df.columns and df["Close"].dropna().size > 0:
    ...
    feige = px.line(
        ...
        x="Date", 
        y="Close", 
        title=f"Kursverlauf ({selected_market})"
    )
    st.plotly_chart(feige, use_container_width=True)
    ...
    st.error(
        f"❌ Keine gültigen Daten für {selected_market} gefunden. "
        "Vielleicht ist die Börse geschlossen oder YFinance liefert gerade nichts zurück."
    )
